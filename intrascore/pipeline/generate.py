...
logInfo = open("/mnt/aix7101/minsuh-output/logInfo_{}_{}_for_clustreing.txt".format(args.model, args.dataset), mode="w",encoding="utf-8")
...
def get_generation_config(input_ids, tokenizer, data_name):
    ...
    if data_name == 'nq':
        generation_config = nq.generate_config(tokenizer)
...
def get_generations(model_name:str, args, seed=1, old_sequences=None, max_num_gen_once=args.num_generations_per_prompt):
    ...
    sequences = []
    for _, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)): # 데이터 하나씩 (batch_size = 1) load
        ...
        while num_gens > 0:
            ...
            generation = dict_outputs.sequences[:, input_length:].cpu() # 질문 (input)은 빼고 모든 generations의 답변 부분만 슬라이싱
...
def get_num_tokens(generation):  # generation: num_seq x max(num_tokens)
    ...
    return num_tokens # 리스트: [gen 1의 토큰 개수, gen 2의 토큰 개수...gen 10의 토큰 개수], len(num_tokens) = 10
